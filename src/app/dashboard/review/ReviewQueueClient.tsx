    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%', maxWidth: '1200px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Button variant="ghost" onClick={handleRefresh} disabled={isRefreshing} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <RefreshCw size={16} className={isRefreshing ? "spin-animation animate-spin" : ""} />
          {isRefreshing ? "Refreshing..." : "Refresh Queue"}
        </Button>
      </div>
      
      {applications.length === 0 ? (
        <Card style={{ padding: '4rem 2rem', textAlign: 'center', color: 'var(--muted-foreground)' }}>
        ...
        <>
          ...
          return (
            <motion.div key={app.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3, ease: "easeOut" }}>
              <Card style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', padding: 0 }}>
                ...
                <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem', cursor: 'pointer', background: 'var(--background)' }} onClick={() => handleToggleExpand(app)}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 1, paddingRight: '1rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        {app.status === 'QUEUED' && <><StatusBadge status="Queued" /><span style={{ fontSize: '0.8rem', color: 'var(--primary)' }}>Agent will apply soon</span></>}
                        {app.status === 'APPLIED' && <><StatusBadge status="Applied" /><span style={{ fontSize: '0.8rem', color: 'var(--success)' }}>Application successful</span></>}
                      </div>
                      <h3 className="font-display" style={{ fontSize: '1.2rem', fontWeight: 600, wordBreak: 'break-word', whiteSpace: 'normal', lineHeight: 1.4, color: 'var(--foreground)' }}>
                    ...
                    >
                      <div style={{ padding: '1.5rem', display: 'grid', gridTemplateColumns: 'minmax(300px, 1fr) 2fr', gap: '2rem', alignItems: 'start' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                          ...
                          <div style={{ padding: '1rem', background: 'var(--success-light)', border: '1px solid var(--success)', borderRadius: '12px' }}>
                            <h4 style={{ color: 'var(--success)', fontSize: '1rem', fontWeight: 600, marginBottom: '0.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                              <Sparkles size={16} /> ATS Match Score
                            </h4>
                            {app.resumeVersion?.atsScore ? (
                              <span style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--foreground)' }}>{app.resumeVersion.atsScore} <span style={{ fontSize: '1rem', color: 'var(--muted-foreground)' }}>/ 100</span></span>
                            ) : (
                              <span style={{ fontSize: '1.2rem', fontWeight: 500, color: 'var(--muted-foreground)', fontStyle: 'italic' }}>Pending Tailoring</span>
                            )}
                          </div>

                          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            <h4 style={{ fontSize: '1.1rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--foreground)' }}>
                            ...
                            {isLoadingAnalysis ? (
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--muted-foreground)', fontSize: '0.9rem' }}>
                                <span className="spinner" style={{ width: '16px', height: '16px', borderWidth: '2px', borderColor: 'var(--primary)', borderTopColor: 'transparent' }}></span> Analyzing changes...
                              </div>
                            ) : (
                              <>
                                {/* Changes Made */}
                                <div style={{ background: 'var(--background)', padding: '1rem', borderRadius: '12px', border: '1px solid var(--border)' }}>
                                  <h5 style={{ fontSize: '0.9rem', color: 'var(--foreground)', marginBottom: '0.75rem', fontWeight: 600 }}>What was tailored:</h5>
                                ...
                                {/* Missing Skills */}
                                <div style={{ background: 'var(--error-light)', padding: '1rem', borderRadius: '12px', border: '1px solid var(--error)' }}>
                                  <h5 style={{ fontSize: '0.9rem', color: 'var(--error)', marginBottom: '0.75rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <AlertTriangle size={14} /> Missing Requirements:
                                  </h5>
                                  <ul style={{ margin: 0, paddingLeft: '1.2rem', fontSize: '0.85rem', color: 'var(--error)', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                    {currentAnalysis?.missingSkills?.length > 0 ? currentAnalysis.missingSkills.map((skill: string, i: number) => (
                                      <li key={i}>{skill}</li>
                                    )) : <li style={{ color: 'var(--success)', listStyle: 'none', marginLeft: '-1.2rem' }}>✨ You meet all requested qualifications!</li>}
                                  </ul>
                                </div>
                              </>
                            )}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', height: '100%' }}>
                          ...
                          <div style={{ display: 'flex', gap: '0.5rem' }}>
                            <Input 
                              ...
                              onKeyDown={(e: any) => e.key === 'Enter' && app.status === 'PENDING_REVIEW' && handleTweak(app.id)}
                            ...
                            >
                              {chatLoading ? <span className="spinner" style={{ width: '18px', height: '18px', borderWidth: '2px' }}></span> : <Send size={18} />}
                            </Button>
                          </div>
                        </div>
                      ...
                      {/* Bottom Action Bar */}
                      <div style={{ padding: '1rem 1.5rem', background: 'var(--background-secondary)', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Button 
                          variant="ghost" 
                          onClick={async () => {
                        ...
                        <div style={{ display: 'flex', gap: '1rem' }}>
                          <Button variant="danger" onClick={() => handleReject(app.id)} disabled={isPending || chatLoading || app.status !== 'PENDING_REVIEW'}>
                            {isPending ? <span className="spinner" style={{ borderWidth: '2px' }}></span> : "Reject"}
                          </Button>
                          <Button variant="primary" onClick={() => handleApprove(app.id)} disabled={isPending || chatLoading || app.status !== 'PENDING_REVIEW'}>
                            {isPending ? <><span className="spinner" style={{ borderWidth: '2px' }}></span> Approving...</> : (app.status === 'PENDING_REVIEW' ? "Approve & Queue Agent" : "Approved ✔")}
                          </Button>
                        </div>
                      </div>
                    </motion.div>
          ...
          {/* Pagination Controls */}
          {totalPages > 1 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem', padding: '1rem', background: 'var(--background)', borderRadius: '12px', border: '1px solid var(--border)' }}>
              <Button 
                variant="ghost" 
